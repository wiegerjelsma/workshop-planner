...
//$a_Timeslots = array();	
//$a_WorkshopsAssigned = array();

// Set the timeslot arrays
//for($i=1; $i<=count($cfg['timeslots']); $i++)
//	$a_Timeslots[$i] = array();
//	
//foreach($cfg['deelnemers'] as $i_Deelnemer => $deelnemer){	
//	foreach($deelnemer['workshops'] as $workshop){ // loop over the workshops for this deelnemer		
//		
//		if(!in_array($workshop, $a_WorkshopsAssigned)){ // if this workshop is not yet added to a timeslot
//			for($i=1; $i<=count($cfg['timeslots']); $i++){ // loop over the 4 timeslots
//								
//				if(count($a_Timeslots[$i]) >= count($cfg['zalen'])) // Als we alle zalen al vol hebben in dit timeslot.
//					continue;	
//							
//				if(isset($cfg['deelnemers'][$i_Deelnemer]['timeslots'][$i])) // Als deze deelnemer al een workshop in dit timeslot heeft
//					continue;			
//								
//				// Add the workshop to this timeslot
//				$a_Timeslots[$i][] = $workshop;
////				print $workshop." added to timeslot ".$i."<br />";
//				$a_WorkshopsAssigned[] = $workshop;
//				$cfg['deelnemers'][$i_Deelnemer]['timeslots'][$i] = $workshop;
//				break;
//			}
//		} else { // This workshop is added to a timeslot
//			
//			// get the timeslot
//			foreach($a_Timeslots as $i => $a_Timeslot)
//				if(in_array($workshop, $a_Timeslot)){
//					$timeslot = $i;
//					break;
//				}
//			
//			// see if the deelnemer already has a workshop in this timeslot
//			if(isset($cfg['deelnemers'][$i_Deelnemer]['timeslots'][$timeslot])){
////				print 'Deelnemer '.$cfg['deelnemers'][$i_Deelnemer]['naam'].' has already a workshop in this timeslot ('.$timeslot.'). Assign the workshop ('.$workshop.') to another timeslot.<br />';
//				
//				print 'Processing: '.$cfg['deelnemers'][$i_Deelnemer]['naam'].'<br />';
//				$reden_DeelnemerHeeftAlWorkshopInDitTimeslot = false;
//				$workshopAssignedToAnotherTimeslot = false;
//				for($i=1; $i<=count($cfg['timeslots']); $i++){ // loop over the 4 timeslots	
//					//print $i.' & '.$timeslot.'<br />';
//					if($i == $timeslot){
//					//	print 'Niet aan waar het al in zit ('.$timeslot.' / '.$workshop.')<br />';
//						continue;
//					} // Dus niet in het timeslot waar het al in zit
//					
//					if(count($a_Timeslots[$i]) >= count($cfg['zalen'])){ // Als we alle zalen al vol hebben in dit timeslot.
//					//	print 'Alle zalen zitten al vol ('.$i.' / '.$workshop.')<br />';
//						continue;
//					}
//						
//					if(isset($cfg['deelnemers'][$i_Deelnemer]['timeslots'][$i])){ // Als deze deelnemer al een workshop in dit timeslot heeft
//					//	print 'Niet als deze deelnemer al een workshop heeft in timeslot ('.$i.' / '.$workshop.')<br />';
//						continue;
//					}
//						
//					// Add the workshop to this timeslot
//					$a_Timeslots[$i][] = $workshop;
////					print $workshop." added (2) to timeslot ".$i."<br />";
//					$cfg['deelnemers'][$i_Deelnemer]['timeslots'][$i] = $workshop;
//					$workshopAssignedToAnotherTimeslot = true;
//					break;					
//				}
//				
//				if(!$workshopAssignedToAnotherTimeslot){
//					print 'We konden <strong>'.$workshop.'</strong> niet aan een ander timeslot koppelen.<br />';					
//					
//					for($i=1; $i<=count($cfg['timeslots']); $i++){
//						if(isset($cfg['deelnemers'][$i_Deelnemer]['timeslots'][$i])){
//							// HIER BEN IK NU
//							
//							
//							// Kunnen we deze deelnemer in dit timeslot aan deze workshop laten deelnemen en de workshop waar hij nu ingedeeld staat dan op een ander timeslot?
//							$workshopForDeelnemerInThisTimeslot = $cfg['deelnemers'][$i_Deelnemer]['timeslots'][$i];
//							print $cfg['deelnemers'][$i_Deelnemer]['naam'].' neemt deel aan '.$workshopForDeelnemerInThisTimeslot.' in timeslot <strong>'.$cfg['timeslots_naming'][$i].'</strong><br />';
//							
//							
//							
//							/*for($j=1; $j<=count($cfg['timeslots']); $j++){ // loop over the 4 timeslots	
//								//print $i.' & '.$timeslot.'<br />';
//								if($i == $j){// Dus niet in het timeslot waar het al in zit
//									print 'Niet aan het huidige timeslot ('.$i.' / '.$j.')<br />';
//									continue;
//								}
//								
//								if(count($a_Timeslots[$j]) >= count($cfg['zalen'])){ // Als we alle zalen al vol hebben in dit timeslot.
//									print 'Alle zalen zitten al vol ('.$j.')<br />';
//									continue;
//								}
//									
//								if(isset($cfg['deelnemers'][$i_Deelnemer]['timeslots'][$j])){ // Als deze deelnemer al een workshop in dit timeslot heeft
//									print 'Hij heeft al een workshop in dit timeslot ('.$j.')<br />';
//									continue;
//								}
//									
//								// Add the workshop to this timeslot
//								$a_Timeslots[$j][] = $workshopForDeelnemerInThisTimeslot;
//								$a_Timeslots[$i][] = $workshop;
//								$cfg['deelnemers'][$i_Deelnemer]['timeslots'][$j] = $workshopForDeelnemerInThisTimeslot;
//								$cfg['deelnemers'][$i_Deelnemer]['timeslots'][$i] = $workshop;
//								$workshopAssignedToAnotherTimeslot = true;								
//print '!! Toch gelukt! <br />';
//								break;					
//							}*/						
//						}
//					}
//					
//					//if(!$workshopAssignedToAnotherTimeslot)
//					//	print 'We konden '.$workshop.' nog steeds niet aan een ander timeslot koppelen.<br />';					
//				}
//
//			}
//			else {
//				$cfg['deelnemers'][$i_Deelnemer]['timeslots'][$timeslot] = $workshop;
//			}
//		}	
//	}
//	if(count($cfg['deelnemers'][$i_Deelnemer]['timeslots']) < 3)
//		print 'We konden niet alle workshops voor  <strong>'.$cfg['deelnemers'][$i_Deelnemer]['naam'].'</strong> assignen'."<br />";			
//}
//
//
////print_r($cfg['deelnemers']);
//
//
//// Fill the timeslot array with empty spaces if neccessary
//foreach($a_Timeslots as $key => $a_Timeslot){
//	for($i=0; $i<count($cfg['zalen']); $i++)
//		$a_Timeslots[$key][$i] = isset($a_Timeslots[$key][$i]) ? $a_Timeslots[$key][$i] : '';
//}
